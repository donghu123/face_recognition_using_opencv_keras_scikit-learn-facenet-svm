...
class Dataset:
    # 很多类都倾向于将对象创建为有初始状态，因此类可能会定义一个名为_init_()的特殊方法（构造方法），类定义了_init()方法的话，类的实例化操作会自动调用_init_()方法。
    # _init_()方法可以有参数，参数通过_init_()传递到类的实例化操作上，比如：path_name,类的方法与普通的函数只有一个特别的区别-它们必须有一个额外的第一个参数名称，按照惯例它的名称是self.
    # self 代表的是类的实例，代表当前对象的地址，而self.class则指向类。
...
class Model:
    ...
    def train(self, dataset, batch_size = 128, nb_epoch = 15, data_augmentation = True):
        sgd = SGD(lr = 0.01, decay = 1e-6,#此处lr可以选择0.001或者0.0001,因为步长过大可能跳过最优解。
